...
<aside id="sidebar" style="width:260px;" class="fixed left-0 top-0 h-full z-50 flex flex-col shadow-2xl"
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
        ...
        @php
            if (!function_exists('sidebarLink')) {
                function sidebarLink($route, $label, $icon, $active = false, $badge = null) {
                    ...
                    $badgeHtml  = $badge
                        ? "<span class=\"collapsible ml-auto text-[10px] font-bold px-2 py-0.5 rounded-full\" style=\"max-width:50px;{$badge['style']}\">{$badge['text']}</span>"
                        : '';
        ...
        {!! sidebarLink('subscription-rates.index', 'Subscription Plans', '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>', request()->routeIs('subscription-rates.*')) !!}

        {!! sidebarLink('sales.index', 'Sales', '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>', request()->routeIs('sales.*'), ['text' => 'New', 'style' => 'background:rgba(16,185,129,.15);color:#6ee7b7;']) !!}

        {!! sidebarLink('billings.index', 'Billing', '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/>', request()->routeIs('billings.*')) !!}

        {!! sidebarLink('payments.index', 'Payments', '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>', request()->routeIs('payments.*')) !!}

        {!! sidebarLink('technicians.index', 'Technicians', '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.7 6.3a4 4 0 01-5.4 5.4l-5.6 5.6a2 2 0 102.8 2.8l5.6-5.6a4 4 0 005.4-5.4z"/>', request()->routeIs('technicians.*')) !!}
        ...
        {!! sidebarLink('users.index', 'User Management', '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/>', request()->routeIs('users.*')) !!}
        ...
        <div class="flex items-center gap-3 p-2 rounded-xl hover:bg-white/[.06] transition-all">
            ...
            <div class="collapsible ml-auto flex-shrink-0" style="max-width:40px;">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" title="Logout" class="w-7 h-7 rounded-lg flex items-center justify-center transition-all hover:bg-red-500/20 hover:scale-105">
