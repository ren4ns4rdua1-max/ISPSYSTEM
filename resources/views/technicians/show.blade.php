...
<div id="main-content" class="flex flex-col flex-1 min-h-screen" style="margin-left:260px;">
    <header class="topbar sticky top-0 z-40 flex items-center justify-between px-7 py-3.5">
        <div class="flex items-center gap-3">
            <a href="{{ route('technicians.index') }}" class="w-8 h-8 bg-gray-100 hover:bg-gray-200 rounded-lg flex items-center justify-center transition-colors">
                <svg class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            </a>
            <div>
                <h1 class="font-display font-bold text-gray-900 text-[20px]">Technician Details</h1>
                <p class="text-gray-400 text-[12px]">View technician information</p>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('technicians.edit', $technician->id) }}" class="px-4 py-2 border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 text-sm">Edit</a>
            <form method="POST" action="{{ route('technicians.destroy', $technician->id) }}" onsubmit="return confirm('Delete this technician?')">
                @csrf @method('DELETE')
                <button type="submit" class="px-4 py-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 text-sm">Delete</button>
            </form>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <div class="flex items-start justify-between mb-6">
                        <div class="flex items-center gap-4">
                            <div class="w-16 h-16 rounded-full bg-gradient-to-br from-red-500 to-red-600 flex items-center justify-center text-white font-bold text-2xl">{{ strtoupper(substr($technician->name, 0, 1)) }}</div>
                            <div>
                                <h2 class="text-xl font-bold text-gray-900">{{ $technician->name }}</h2>
                                <p class="text-gray-500">{{ $technician->email }}</p>
                                <span class="mt-2 inline-flex px-3 py-1 rounded-full text-xs font-semibold {{ $technician->status_badge }}">{{ ucfirst($technician->status) }}</span>
                            </div>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        <div><p class="text-xs text-gray-400 uppercase">Phone</p><p class="font-medium">{{ $technician->phone_number }}</p></div>
                        <div><p class="text-xs text-gray-400 uppercase">Specialization</p><p class="font-medium">{{ ucfirst($technician->specialization ?? 'N/A') }}</p></div>
                        <div><p class="text-xs text-gray-400 uppercase">Area Coverage</p><p class="font-medium">{{ $technician->area_coverage ?? 'All Areas' }}</p></div>
                    </div>
                    @if($technician->notes)
                    <div class="mt-4 pt-4 border-t border-gray-100">
                        <p class="text-xs text-gray-400 uppercase">Notes</p>
                        <p class="text-gray-600 mt-1">{{ $technician->notes }}</p>
                    </div>
                    @endif
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100"><h3 class="font-semibold text-gray-800">Active Jobs</h3></div>
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr>
                                <td class="px-6 py-4">
                                    <p class="font-medium text-gray-900">{{ $job->client->name }}</p>
                                    <p class="text-gray-500 text-sm">{{ $job->client->barangay }}</p>
                                </td>
                                <td class="px-6 py-4 text-gray-600">{{ $job->job_type_label }}</td>
                                <td class="px-6 py-4"><span class="px-2 py-1 rounded-full text-xs {{ $job->status_badge }}">{{ ucfirst(str_replace('_', ' ', $job->status)) }}</span></td>
                                <td class="px-6 py-4 text-gray-600">{{ $job->scheduled_date?->format('M d, Y') ?? 'Not scheduled' }}</td>
                                <td class="px-6 py-4">
                                    @if($job->status == 'assigned')
                                    <form method="POST" action="{{ route('technicians.start-job', $job->id) }}" class="inline">
                                        @csrf
                                        <button type="submit" class="text-amber-600 hover:text-amber-800 text-sm font-medium">Start</button>
                                    </form>
                                    @endif
                                    @if($job->status == 'in_progress')
                                    <form method="POST" action="{{ route('technicians.complete-job', $job->id) }}" class="inline">
                                        @csrf
                                        <button type="submit" class="text-emerald-600 hover:text-emerald-800 text-sm font-medium">Complete</button>
                                    </form>
                                    @endif
                                </td>
                            </tr>
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <h3 class="font-semibold text-gray-800 mb-4">Performance</h3>
                    <div class="space-y-4">
                        <div class="flex justify-between items-center">
                            <span class="text-gray-600 text-sm">Pending Jobs</span>
                            <span class="font-bold text-amber-600">{{ $pendingJobs->count() }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-gray-600 text-sm">Completed Jobs</span>
                            <span class="font-bold text-emerald-600">{{ $completedJobs }}</span>
                        </div>
                    </div>
                </div>
            </div>
