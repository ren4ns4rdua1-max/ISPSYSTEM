...
<aside id="sidebar" style="width:260px;" class="fixed left-0 top-0 h-full z-50 flex flex-col shadow-2xl">
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
        ...
        <!-- Technicians - ACTIVE -->
        <div class="nav-wrapper relative">
            <a href="{{ route('technicians.index') }}" class="nav-item-inner relative flex items-center gap-3 px-3 py-2.5 rounded-xl"
               style="background:linear-gradient(135deg,rgba(220,38,38,.18),rgba(185,28,28,.12));border:1px solid rgba(220,38,38,.28);">
                ...
                <span class="collapsible text-sm font-semibold text-red-200" style="max-width:160px;">Technicians</span>
            </a>
...
<div id="main-content" class="flex flex-col flex-1 min-h-screen" style="margin-left:260px;">
    <header class="topbar sticky top-0 z-40 flex items-center justify-between px-7 py-3.5">
        <div class="flex items-center gap-3">
            <div>
                <h1 class="font-display font-bold text-gray-900 text-[20px]">Technicians</h1>
                <p class="text-gray-400 text-[12px]">Manage your installation and repair team</p>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('technicians.create') }}" class="px-4 py-2 text-white rounded-xl font-medium text-sm" style="background:linear-gradient(135deg,#dc2626,#b91c1c);">+ Add Technician</a>
            <div class="w-9 h-9 rounded-xl avatar-grad flex items-center justify-center text-white font-bold text-sm">{{ strtoupper(substr(Auth::user()->name, 0, 1)) }}</div>
        </div>
    </header>
    <main class="flex-1 p-6">
        @if(session('success'))<div class="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg mb-6">{{ session('success') }}</div>@endif
        
        <!-- Stats Cards -->
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                <p class="text-gray-500 text-sm">Total Technicians</p>
                <p class="font-display font-bold text-2xl text-gray-900">{{ $stats['total_technicians'] }}</p>
            </div>
            <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                <p class="text-gray-500 text-sm">Available</p>
                <p class="font-display font-bold text-2xl text-emerald-600">{{ $stats['available'] }}</p>
            </div>
            <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                <p class="text-gray-500 text-sm">Busy</p>
                <p class="font-display font-bold text-2xl text-amber-600">{{ $stats['busy'] }}</p>
            </div>
            <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                <p class="text-gray-500 text-sm">Pending Jobs</p>
                <p class="font-display font-bold text-2xl text-red-600">{{ $stats['pending_jobs'] }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 mb-6">
            <form method="GET" class="flex gap-4">
                <input type="text" name="search" value="{{ $search }}" placeholder="Search technicians..." class="flex-1 px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-red-300">
                <select name="status" class="px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-red-300">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Technician</th>
                        <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Contact</th>
                        <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Specialization</th>
                        <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Area</th>
                        <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Status</th>
                        <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Active Jobs</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-gradient-to-br from-red-500 to-red-600 flex items-center justify-center text-white font-semibold">{{ substr($technician->name, 0, 1) }}</div>
                                <div><p class="font-medium text-gray-900">{{ $technician->name }}</p><p class="text-gray-500 text-sm">{{ $technician->email }}</p></div>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-gray-600">{{ $technician->phone_number }}</td>
                        <td class="px-6 py-4 text-gray-600">{{ ucfirst($technician->specialization ?? 'N/A') }}</td>
                        <td class="px-6 py-4 text-gray-600">{{ $technician->area_coverage ?? 'All Areas' }}</td>
                        <td class="px-6 py-4"><span class="px-3 py-1 rounded-full text-xs font-medium {{ $technician->status_badge }}">{{ ucfirst($technician->status) }}</span></td>
                        <td class="px-6 py-4"><span class="font-medium text-gray-900">{{ $technician->jobs_count }}</span></td>
                        <td class="px-6 py-4">
                            <div class="flex gap-2">
                                <a href="{{ route('technicians.show', $technician->id) }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">View</a>
                                <a href="{{ route('technicians.edit', $technician->id) }}" class="text-gray-600 hover:text-gray-800 text-sm font-medium">Edit</a>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="7" class="px-6 py-8 text-center text-gray-500">No technicians found. <a href="{{ route('technicians.create') }}" class="text-red-600 hover:underline">Add one now</a></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
