...
<script>
    ...
    function updatePlanDetails() {
        const select = document.getElementById("subscription_rate_id");
        const option = select.options[select.selectedIndex];
        const planDesc = document.getElementById("plan_description");
        if (option.value) {
            if (!planDesc.value) {
                planDesc.value = option.text.split(" - ")[0];
            }
            calculateTotal();
        }
    }
    ...
    function calculateTotal() {
        ...
        const monthlyFee = parseFloat(option.dataset.monthly) || 0;
        const installFee = parseFloat(option.dataset.install) || 0;
        ...
        if (billingType === "monthly") {
            ...
            baseAmount = monthlyFee * 12;
        } else if (billingType === "installation") {
            baseAmount = installFee;
        }
        
        const total = baseAmount + tax - discount;
        
        document.getElementById("baseAmount").textContent = "₱" + baseAmount.toLocaleString("en-PH", {minimumFractionDigits: 2, maximumFractionDigits: 2});
        document.getElementById("taxDisplay").textContent = "₱" + tax.toLocaleString("en-PH", {minimumFractionDigits: 2, maximumFractionDigits: 2});
        document.getElementById("discountDisplay").textContent = "-₱" + discount.toLocaleString("en-PH", {minimumFractionDigits: 2, maximumFractionDigits: 2});
