...
<div id="main-content" class="flex flex-col flex-1 min-h-screen">
    ...
    <main class="flex-1 main-scroll p-6">
        <form method="POST" action="{{ route('billings.store') }}" class="max-w-7xl mx-auto">
            @csrf
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="form-card bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <div class="p-6">
                                        ...
                                        <select name="client_id" id="client_id" required class="form-input">
                                            ...
                                            @foreach($clients as $client)
                                                <option value="{{ $client->id }}"
                                                    data-plan="{{ $client->plan_description }}"
                                                    data-email="{{ $client->email }}"
                                                    data-phone="{{ $client->phone_number }}"
                                                    {{ old('client_id', $prefillClientId ?? '') == $client->id ? 'selected' : '' }}>
                                                    {{ $client->name }} — {{ $client->plan_description }} ({{ $client->email }})
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="input-wrapper-select">
                                        ...
                                        <select name="subscription_rate_id" id="subscription_rate_id" class="form-input">
                                            ...
                                            @foreach($subscriptionRates as $rate)
                                                <option value="{{ $rate->id }}" data-price="{{ $rate->monthly_fee }}" data-plan="{{ $rate->plan_name }}">
                            ...
                            <div id="client-details" class="client-details-panel mt-5 hidden">
                                <!-- Auto-filled notice -->
                                <div class="flex items-center gap-2 mb-3 px-3 py-2 rounded-xl text-xs font-semibold" style="background:#f0fdf4;color:#059669;border:1px solid #d1fae5;">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                    Form auto-filled from client subscription data
                                </div>
                                <!-- Plan highlight card -->
                                <div id="plan-highlight" class="mb-3 px-4 py-3 rounded-xl hidden" style="background:linear-gradient(135deg,#fef2f2,#fff5f5);border:1px solid #fecaca;">
                                    <p class="text-[10px] font-bold text-red-400 uppercase tracking-wider mb-1">Subscribed Plan</p>
                                    <p id="plan-highlight-name" class="font-bold text-red-700 text-sm"></p>
                                    <p id="plan-highlight-fee" class="text-red-500 text-xs mt-0.5"></p>
                                </div>
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm p-4 bg-gray-50 rounded-xl">
                                    <div>
                                        <p class="text-gray-400 text-xs font-semibold uppercase tracking-wider">PPPoE Name</p>
                                        <p id="detail-pppoe" class="font-semibold text-gray-800 mt-1">-</p>
                                    </div>
                                    <div>
                                        <p class="text-gray-400 text-xs font-semibold uppercase tracking-wider">Phone</p>
                                        <p id="detail-phone" class="font-semibold text-gray-800 mt-1">-</p>
                                    </div>
                                    <div>
                                        <p class="text-gray-400 text-xs font-semibold uppercase tracking-wider">Barangay</p>
                                        <p id="detail-barangay" class="font-semibold text-gray-800 mt-1">-</p>
                                    </div>
                                    <div>
                                        <p class="text-gray-400 text-xs font-semibold uppercase tracking-wider">Current Plan</p>
                                        <p id="detail-plan" class="font-semibold text-gray-800 mt-1">-</p>
                                    </div>
                                </div>
                                <p id="outstanding-warn" class="hidden mt-3 text-xs font-semibold text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2"></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-card bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <div class="p-6">
                            <textarea name="notes" rows="3" class="form-input" placeholder="Additional notes about this invoice...">{{ old('notes') }}</textarea>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <div class="input-wrapper">
                                        ...
                                        <input type="number" name="amount" id="amount" value="{{ old('amount', 0) }}" step="0.01" min="0" required class="form-input" onchange="calculateTotal()">
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div class="input-wrapper">
                                        ...
                                        <input type="number" name="tax_amount" id="tax_amount" value="{{ old('tax_amount', 0) }}" step="0.01" min="0" class="form-input" onchange="calculateTotal()">
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="input-wrapper">
                                        ...
                                        <input type="number" name="discount_amount" id="discount_amount" value="{{ old('discount_amount', 0) }}" step="0.01" min="0" class="form-input" onchange="calculateTotal()">
                                    </div>
                                </div>
                                ...
                                <div class="pt-4 border-t border-gray-100">
                                    <div class="flex justify-between items-center">
                                        ...
                                        <span id="total-display" class="text-xl font-bold text-red-600">₱0.00</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <div class="input-wrapper">
                                        ...
                                        <input type="date" name="paid_date" value="{{ old('paid_date') }}" class="form-input">
                                    </div>
                                </div>
...
<script>
    ...
    const planData = {
        ...
        "{{ $rate->id }}": {
            ...
            name:         "{{ addslashes($rate->plan_name) }}",
            speed:        "{{ addslashes($rate->speed ?? '') }}"
        },
        @endforeach
    };
    ...
    document.getElementById('client_id').addEventListener('change', function() {
        ...
        fetch('/billings/client/' + id, {
            headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': CSRF }
        })
        .then(r => r.json())
        .then(data => {
            // Show info panel
            document.getElementById('detail-pppoe').textContent    = data.pppoe_name    || '-';
            document.getElementById('detail-phone').textContent    = data.phone_number  || '-';
            document.getElementById('detail-barangay').textContent = data.barangay      || '-';
            document.getElementById('detail-plan').textContent     = data.plan_name     || data.plan_description || '-';
            panel.classList.remove('hidden');

            const planSel = document.getElementById('subscription_rate_id');

            // Step 1: try to set by subscription_rate_id returned from server
            let matched = false;
            if (data.subscription_rate_id) {
                const opt = planSel.querySelector(`option[value="${data.subscription_rate_id}"]`);
                if (opt) {
                    planSel.value = data.subscription_rate_id;
                    matched = true;
                }
            }

            // Step 2: fallback — match by plan name text inside the option labels
            if (!matched && data.plan_name) {
                const needle = data.plan_name.toLowerCase();
            ...
            // Step 3: fallback — match by plan_description text
            if (!matched && data.plan_description) {
                const needle = data.plan_description.toLowerCase().split(' ')[0]; // first word
                for (const opt of planSel.options) {
                    if (opt.value && opt.text.toLowerCase().includes(needle)) {
                        planSel.value = opt.value;
                        matched = true;
            ...
            if (matched && planSel.value) {
                // Highlight the plan dropdown
                planSel.style.borderColor = '#dc2626';
                planSel.style.background  = '#fff5f5';

                // Show plan highlight card
                const plan = planData[String(planSel.value)];
                if (plan) {
                    const highlight = document.getElementById('plan-highlight');
                    document.getElementById('plan-highlight-name').textContent =
                        plan.name + (plan.speed ? ' \u2014 ' + plan.speed : '');
                    document.getElementById('plan-highlight-fee').textContent =
                        'Monthly fee: \u20b1' + parseFloat(plan.monthly).toLocaleString('en-PH', { minimumFractionDigits: 2 });
                    highlight.classList.remove('hidden');
                }

                fillAmountFromPlan(planSel.value);
            } else {
                // No plan matched — use monthly_fee directly if available
                planSel.style.borderColor = '';
                planSel.style.background  = '';
                document.getElementById('plan-highlight').classList.add('hidden');
                if (data.monthly_fee && parseFloat(data.monthly_fee) > 0) {
                    document.getElementById('amount').value = parseFloat(data.monthly_fee).toFixed(2);
                    calculateTotal();
                }
            }
            ...
            if (data.due_date_time) {
                document.querySelector('input[name="due_date"]').value = data.due_date_time;
            }

            // Outstanding warning
            const warn = document.getElementById('outstanding-warn');
            if (warn) {
                if (data.outstanding > 0) {
                    warn.textContent = 'Warning: This client has ' + data.outstanding + ' unpaid invoice(s).';
    ...
    // ── Fill amount based on plan + billing type ───────────────────────
    function fillAmountFromPlan(planId) {
        const plan = planData[String(planId)];
        if (!plan) return;
        const billingType = document.querySelector('select[name="billing_type"]').value;
        let amount = 0;
        if (billingType === 'monthly')           amount = plan.monthly;
        else if (billingType === 'quarterly')    amount = plan.monthly * 3;
        else if (billingType === 'yearly')       amount = plan.monthly * 12;
        else if (billingType === 'installation') amount = plan.installation;
        else                                     amount = plan.monthly;
        document.getElementById('amount').value = parseFloat(amount).toFixed(2);
        calculateTotal();
    }

    // ── Plan dropdown changed manually ────────────────────────────────
    document.getElementById('subscription_rate_id').addEventListener('change', function() {
        ...
        const plan = planData[String(this.value)];
        if (plan) {
        ...
        fillAmountFromPlan(this.value);
    });

    // ── Billing type changed → recalculate ────────────────────────────
    document.querySelector('select[name="billing_type"]').addEventListener('change', function() {
        const planId = document.getElementById('subscription_rate_id').value;
        if (planId) fillAmountFromPlan(planId);
        else calculateTotal();
    });
    ...
    // Trigger if client pre-selected via ?client_id=
