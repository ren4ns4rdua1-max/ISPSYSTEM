...
<div id="main-content" class="flex flex-col flex-1 min-h-screen" style="margin-left:260px;">
    <header class="topbar sticky top-0 z-40 flex items-center justify-between px-7 py-3.5">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('billings.edit', $billing->id) }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white rounded-xl shadow-md hover:shadow-lg transition-all" style="background:linear-gradient(135deg,#dc2626,#b91c1c);">
    ...
    <main class="flex-1 p-6">
        @if (session('success'))
            <div class="flex items-center gap-3 px-5 py-4 rounded-xl border mb-6" style="background:#f0fdf4;border-color:#bbf7d0;">
                <div class="w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center"><svg class="w-4 h-4 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg></div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    <div class="flex items-start justify-between mb-6">
                        ...
                        @switch($billing->status)
                            @case('paid')
                                <span class="px-4 py-2 rounded-full text-sm font-semibold bg-green-100 text-green-700">Paid</span>
                                @break
                            @case('pending')
                                <span class="px-4 py-2 rounded-full text-sm font-semibold bg-yellow-100 text-yellow-700">Pending</span>
                                @break
                            @case('overdue')
                                <span class="px-4 py-2 rounded-full text-sm font-semibold bg-red-100 text-red-700">Overdue</span>
                                @break
                            @case('partial')
                                <span class="px-4 py-2 rounded-full text-sm font-semibold bg-blue-100 text-blue-700">Partial</span>
                                @break
                            @case('cancelled')
                                <span class="px-4 py-2 rounded-full text-sm font-semibold bg-gray-100 text-gray-500">Cancelled</span>
                                @break
                        @endswitch
                    </div>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    <h3 class="font-display font-bold text-gray-800 text-lg mb-4">Payment Details</h3>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    <h3 class="font-display font-bold text-gray-800 text-lg mb-4">Payment Records</h3>
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Receipt #</th>
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    <h3 class="font-display font-bold text-gray-800 text-lg mb-4">Amount Summary</h3>
                    <div class="space-y-3">
                        <div class="flex justify-between">
                            <span class="text-gray-500">Subtotal</span>
                            <span class="font-semibold text-gray-900">₱{{ number_format($billing->amount, 2) }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-500">Tax</span>
                            <span class="font-semibold text-gray-900">₱{{ number_format($billing->tax_amount, 2) }}</span>
                        </div>
                        <div class="flex justify-between">
                        ...
                        <div class="pt-3 border-t border-gray-100 flex justify-between">
                            ...
                            <span class="font-bold text-xl text-gray-900">₱{{ number_format($billing->total_amount, 2) }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    <h3 class="font-display font-bold text-gray-800 text-lg mb-4">Quick Actions</h3>
                    <form method="POST" action="{{ route('billings.markAsPaid', $billing->id) }}" class="space-y-3">
                        @csrf
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 mb-1">Paid Date</label>
                            <input type="date" name="paid_date" value="{{ date('Y-m-d') }}" class="w-full text-sm bg-gray-50 rounded-lg px-3 py-2 border-0 focus:outline-none focus:ring-2 focus:ring-red-300">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 mb-1">Payment Method</label>
                            <select name="payment_method" class="w-full text-sm bg-gray-50 rounded-lg px-3 py-2 border-0 focus:outline-none focus:ring-2 focus:ring-red-300">
                ...
                <!-- Meta Info -->
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    <h3 class="font-display font-bold text-gray-800 text-lg mb-4">Information</h3>
                    <div class="space-y-3 text-sm">
                        <div>
                            <p class="text-gray-400 text-xs">Created By</p>
                            <p class="font-semibold text-gray-800">{{ $billing->creator->name ?? 'System' }}</p>
                        </div>
                        <div>
                            <p class="text-gray-400 text-xs">Created At</p>
                            <p class="font-semibold text-gray-800">{{ $billing->created_at->format('M d, Y g:i A') }}</p>
                        </div>
                        <div>
                            <p class="text-gray-400 text-xs">Last Updated</p>
                            <p class="font-semibold text-gray-800">{{ $billing->updated_at->format('M d, Y g:i A') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <!-- Delete -->
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    <button onclick="confirmDelete()" class="w-full px-4 py-2.5 text-sm font-semibold text-red-600 bg-red-50 rounded-xl hover:bg-red-100 transition-all">
                        Delete Invoice
                    </button>
                </div>
            </div>
...
<div id="delete-modal" class="fixed inset-0 z-50 items-center justify-center" style="background:rgba(0,0,0,0.5);backdrop-filter:blur(4px);display:none;">
    <div class="bg-white rounded-2xl shadow-2xl p-6 mx-4 w-full max-w-sm">
        ...
        <div class="flex items-center gap-3">
            <button onclick="closeModal()" class="flex-1 px-4 py-2.5 text-sm font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-xl">Cancel</button>
            <form method="POST" action="{{ route('billings.destroy', $billing->id) }}" class="flex-1">@csrf @method('DELETE')<button type="submit" class="w-full px-4 py-2.5 text-sm font-semibold text-white rounded-xl" style="background:linear-gradient(135deg,#dc2626,#b91c1c);">Yes, Delete</button></form>
        </div>
    </div>
</div>
