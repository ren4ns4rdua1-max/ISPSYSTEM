...
<aside id="sidebar" style="width:260px;" class="fixed left-0 top-0 h-full z-50 flex flex-col shadow-2xl">
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
        ...
        <!-- Payments (Active) -->
        <div class="nav-wrapper relative">
            <a href="{{ route('payments.index') }}" class="nav-item-inner flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all"
               style="background:linear-gradient(135deg,rgba(220,38,38,.12),rgba(185,28,28,.08));border:1px solid rgba(220,38,38,.2);">
                ...
                <span class="collapsible text-sm font-semibold text-red-300" style="max-width:160px;">Payments</span>
            </a>
...
<div id="main-content" class="flex flex-col flex-1 min-h-screen">
    ...
    <header class="topbar sticky top-0 z-40 flex items-center justify-between px-7 py-3.5">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('payments.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 text-white font-semibold text-sm rounded-xl shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300"
               style="background:linear-gradient(135deg,#dc2626,#b91c1c);">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <main class="flex-1 main-scroll p-6 space-y-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-gray-400 text-[11px] font-semibold uppercase tracking-wider">Transactions</p>
                        <p class="font-display font-extrabold text-gray-900 text-2xl mt-1">{{ $stats['transactions_count'] }}</p>
                    </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            @if($payments->isEmpty())
                ...
                <div class="flex flex-col items-center justify-center py-20 px-6 text-center">
                    ...
                    <p class="text-gray-400 text-sm mb-6 max-w-xs">Record your first payment to get started.</p>
                    <a href="{{ route('payments.create') }}" class="inline-flex items-center gap-2 px-6 py-3 text-white font-semibold text-sm rounded-xl transition-all hover:shadow-lg"
                       style="background:linear-gradient(135deg,#dc2626,#b91c1c);">
                    ...
                    <table class="w-full">
                        <thead>
                            <tr style="background:linear-gradient(90deg,#fef2f2,#fff5f5);">
                                <th class="px-6 py-4 text-left text-[11px] font-bold text-gray-500 uppercase tracking-wider">Receipt #</th>
                                ...
                                <th class="px-6 py-4 text-left text-[11px] font-bold text-gray-500 uppercase tracking-wider">Method</th>
                            ...
                            @foreach($payments as $payment)
                                <tr class="trow" style="animation-delay: {{ $loop->index * 40 }}ms;">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm font-bold text-gray-900">{{ $payment->receipt_number }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div>
                                            <p class="text-sm font-semibold text-gray-900">{{ $payment->client->name }}</p>
                                            <p class="text-[10px] text-gray-400">{{ $payment->client->email }}</p>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($payment->billing)
                                            <span class="text-sm text-gray-600">{{ $payment->billing->invoice_number }}</span>
                                        @else
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium badge-{{ $payment->payment_method }}">
                                            {{ $payment->payment_method_label }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm text-gray-600">{{ \Carbon\Carbon::parse($payment->payment_date)->format('M d, Y') }}</span>
                                    </td>
                ...
                @if ($payments->hasPages())
                    <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between flex-wrap gap-3">
                        <p class="text-[12px] text-gray-500">
                            ...
                            of <span class="font-semibold text-gray-800">{{ $payments->total() }}</span> payments
                        </p>
                        {{ $payments->links() }}
                    </div>
                @endif
